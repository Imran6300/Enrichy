...
export default function NewJobPage() {
  ...
  useEffect(() => {
    ...
    // failed job-creation round trip. Super admin accounts (see
    // db/models/User.js) ignore this entirely, regardless of past usage.
  ...
  const hasAiKey = Boolean(aiCreds?.apiKey) || isSuperAdmin;
  ...
  async function handleSubmit(e) {
    ...
    if (!hasAiKey) {
      setError(
        "Add your LLM provider and API key in Settings before starting a run.",
      );
      return;
    }
    if (freeRunUsed) {
    ...
    setSubmitting(true);
    try {
      const clampedTarget = isSuperAdmin
        ? target
        : Math.min(target, MAX_LEADS_PER_RUN);
      ...
      if (err.status === 403 && err.body?.upsell) {
        ...
        if (err.body.upsell.gumroadUrl)
          setGumroadUrl(err.body.upsell.gumroadUrl);
        setShowUpsell(true);
      } else {
    ...
    <div className="animate-fade-in-up">
      <div className="page-head">
        <div>
          ...
          <p>
            Discover, crawl and verify leads for any business type in any city —
      ...
      {!hasAiKey && (
        <div className="error-banner mb-4">
          ...
          — it's required to run the AI verification step, and your key is only
          ever used for your own run.
        </div>
      )}
      ...
      {freeRunUsed && (
        <div className="info-banner">
          ...
          <span>
            You've already used your free run on this account. Get the full
            source code to run unlimited enrichments on your own server —{" "}
            <button
              type="button"
              className="underline"
      ...
      <Modal open={showUpsell} onClose={() => setShowUpsell(false)}>
        ...
        <p className="mb-4 text-[13.5px] leading-relaxed text-inkDim">
          {upsellMessage}
        </p>
      ...
      <div className="grid grid-cols-1 items-start gap-[18px] lg:grid-cols-[1.1fr_0.9fr]">
        <form className="panel" onSubmit={handleSubmit}>
          ...
          <div className="field-group">
            ...
            <p className="field-hint">
              Press Enter or comma to add. Every keyword is combined with every
              city below.
            </p>
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">
                {suggestions.exampleKeywords.slice(0, 6).map((k) => (
                  <button
                    ...
                    key={k}
                    onClick={() =>
                      !keywords.includes(k) && setKeywords([...keywords, k])
                    }
                  >
                    + {k}
                  </button>
          ...
          <div className="field-group">
            ...
            <TagInput
              value={cities}
              onChange={setCities}
              placeholder="e.g. Pune, Jaipur, Nagpur…"
            />
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">
                {suggestions.exampleCities.slice(0, 8).map((c) => (
                  <button
                    ...
                    key={c}
                    onClick={() =>
                      !cities.includes(c) && setCities([...cities, c])
                    }
                  >
                    + {c}
                  </button>
          ...
          <div className="field-group">
            ...
            <input
              type="number"
              min="1"
              max={isSuperAdmin ? undefined : MAX_LEADS_PER_RUN}
              value={target}
              onChange={(e) =>
                setTarget(
                  isSuperAdmin
                    ? Number(e.target.value)
                    : Math.min(Number(e.target.value), MAX_LEADS_PER_RUN),
                )
              }
            ...
            <p className="field-hint">
              {isSuperAdmin
                ? "The run stops discovering once this many new leads have been found."
        ...
        <div className="panel">
          ...
          <div className="text-[13.5px] leading-relaxed text-inkDim">
            <p className="mb-3">
              <strong className="text-ink">1. Discover</strong> — searches the
              web for businesses matching each keyword × city combination.
            </p>
            <p className="mb-3">
              <strong className="text-ink">2. Enrich</strong> — crawls each
              business website to extract contacts, emails, phone and address.
            </p>
            <p className="mb-0">
              <strong className="text-ink">3. Verify</strong> — runs every
              resolved email through the 12-check self-hosted verification
              engine.
            </p>
            {combos > 0 && (
