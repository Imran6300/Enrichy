    ...
    <NavLink
      ...
      className={({ isActive }) =>
        `flex items-center gap-2.5 rounded-md px-2.5 py-2.5 text-[13.5px] font-medium no-underline transition-colors duration-150 ${
          isActive
            ? "bg-panelRaised text-amber"
            : "text-inkDim hover:bg-panelRaised hover:text-ink"
        }`
      }
    >
      <span className="w-4 text-center font-mono text-xs opacity-80">
        {icon}
      </span>
      {label}
    </NavLink>
...
export default function App() {
  ...
  return (
    <div className="grid min-h-screen grid-cols-1 content-start lg:content-stretch lg:grid-cols-[236px_1fr]">
